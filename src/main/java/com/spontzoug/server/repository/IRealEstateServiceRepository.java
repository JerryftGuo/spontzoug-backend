...
public interface IRealEstateServiceRepository extends ReactiveMongoRepository<RealEstateService,String> {
    ...
    Flux<RealEstateService> findByRegion(final String region );
    @Query(value="{'region':?0,'modified':{'gt':?1 }",fields="{'created':0,'modified':0,'creator':0}")
    Flux<RealEstateService> findByRegionInc(final String region, final Date date );
