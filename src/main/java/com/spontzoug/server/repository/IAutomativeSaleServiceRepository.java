...
public interface IAutomativeSaleServiceRepository extends ReactiveMongoRepository<AutomativeSaleService,String> {
    ...
    Flux<AutomativeSaleService> findByRegion(final String region );
    @Query(value="{'region':?0,'modified':{'gt':?1 }",fields="{'created':0,'modified':0,'creator':0}")
    Flux<AutomativeSaleService> findByRegionInc(final String region, final Date date );
