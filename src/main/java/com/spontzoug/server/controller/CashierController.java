...
import com.spontzoug.server.service.ICashierService;
...
public class CashierController {
    ...
    public void create(@RequestBody Cashier l) {  cashierService.create(l); }

    @PutMapping("/update")
    @ResponseStatus(HttpStatus.OK)
    public Mono<Cashier> update(@RequestBody Cashier m) {
        return cashierService.update(m);
    }
    ...
    @GetMapping("/id/{id}")
    public ResponseEntity<Mono<Cashier>> findById(@PathVariable("id") String id) {
        Mono<Cashier> m = cashierService.findById(id);
        HttpStatus s = m!=null ? HttpStatus.OK : HttpStatus.NOT_FOUND;
        return new ResponseEntity<Mono<Cashier>>(m,s);
    }

    @GetMapping("/businessid/{id}")
    public Flux<Cashier> findByBusinessid(@PathVariable("id") String id){
        return cashierService.findByBusinessid(id);
    }
    @GetMapping("/businessconfig/{id}")
    public Flux<Cashier> findByBusinessConf(@PathVariable("id") String id){
        return cashierService.findByBusinessid(id);
    }
    @GetMapping("/businessid/{id}/{date}")
    public Flux<Cashier> findByBusinessidInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
        return cashierService.findByBusinessidInc(id,date);
    }


    @GetMapping("/dynamic/{id}")
    public Flux<Cashier> findDynamics(@PathVariable("id") String id){
        return cashierService.findDynamics(id);
    }
    @GetMapping("/dynamic/{id}/{date}")
    public Flux<Cashier> findDynamicsInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
        return cashierService.findDynamicsInc(id,date);
    }
    ...
    public Mono<ClosingResponse> closing(@RequestBody ClosingRequest request) {
        ...
        return cashierService.findById(id)
                .flatMap( user ->{
                    ...
                    return cashierService.update(user)
                            .flatMap( user1 ->{
