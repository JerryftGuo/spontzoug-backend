...
import com.spontzoug.server.service.IPractitionerService;
...
public class PractitionerController {
    ...
    public void create(@RequestBody Practitioner l) {  practitionerService.create(l); }

    @PutMapping("/update")
    @ResponseStatus(HttpStatus.OK)
    public Mono<Practitioner> update(@RequestBody Practitioner m) {
        return practitionerService.update(m);
    }

    @DeleteMapping("/delete/{id}")
    @ResponseStatus(HttpStatus.OK)
    public Mono<Void> deleteById (@PathVariable("id") String id) {
        return practitionerService.deleteById(id);
    }

    @GetMapping("/id/{id}")
    public ResponseEntity<Mono<Practitioner>> findById(@PathVariable("id") String id) {
        Mono<Practitioner> m = practitionerService.findById(id);
        HttpStatus s = m!=null ? HttpStatus.OK : HttpStatus.NOT_FOUND;
        return new ResponseEntity<Mono<Practitioner>>(m,s);
    }

    @GetMapping("/businessid/{id}")
    public Flux<Practitioner> findByBusinessid(@PathVariable("id") String id){
        return practitionerService.findByBusinessid(id);
    }
    @GetMapping("/businessconfig/{id}")
    public Flux<Practitioner> findByBusinessConf(@PathVariable("id") String id){
        return practitionerService.findByBusinessid(id);
    }
    @GetMapping("/businessid/{id}/{date}")
    public Flux<Practitioner> findByBusinessidInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
        return practitionerService.findByBusinessidInc(id,date);
    }

    @GetMapping("/dynamic/{id}")
    public Flux<Practitioner> findDynamics(@PathVariable("id") String id){
        return practitionerService.findDynamics(id);
    }
    @GetMapping("/dynamic/{id}/{date}")
    public Flux<Practitioner> findDynamicsInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
        return practitionerService.findDynamicsInc(id,date);
    }
    ...
    public Mono<ClosingResponse> closing(@RequestBody ClosingRequest request) {
        ...
        return practitionerService.findById(id)
                .flatMap( user ->{
                    ...
                    return practitionerService.update(user)
                            .flatMap( user1 ->{
