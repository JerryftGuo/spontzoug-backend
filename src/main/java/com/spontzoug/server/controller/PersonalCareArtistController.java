...
import com.spontzoug.server.service.IPersonalCareArtistService;
...
@RestController
@RequestMapping("/api/personalcareartist")
public class PersonalCareArtistController {
    @Autowired
    private IPersonalCareArtistService personalCareArtistService;
    @Autowired
    ...
    public void create(@RequestBody PersonalCareArtist l) {  personalCareArtistService.create(l); }

    @PutMapping("/update")
    @ResponseStatus(HttpStatus.OK)
    public Mono<PersonalCareArtist> update(@RequestBody PersonalCareArtist m) {
    ...
    @GetMapping("/id/{id}")
    public ResponseEntity<Mono<PersonalCareArtist>> findById(@PathVariable("id") String id) {
        Mono<PersonalCareArtist> m = personalCareArtistService.findById(id);
        HttpStatus s = m!=null ? HttpStatus.OK : HttpStatus.NOT_FOUND;
        return new ResponseEntity<Mono<PersonalCareArtist>>(m,s);
    }

    @GetMapping("/businessid/{id}")
    public Flux<PersonalCareArtist> findByBusinessid(@PathVariable("id") String id){
        return personalCareArtistService.findByBusinessid(id);
    }
    @GetMapping("/businessconfig/{id}")
    public Flux<PersonalCareArtist> findByBusinessConf(@PathVariable("id") String id){
        return personalCareArtistService.findByBusinessid(id);
    }
    @GetMapping("/businessid/{id}/{date}")
    public Flux<PersonalCareArtist> findByBusinessidInc(
    ...
    public Flux<PersonalCareArtist> findDynamics(@PathVariable("id") String id){
        return personalCareArtistService.findDynamics(id);
    }
    @GetMapping("/dynamic/{id}/{date}")
    public Flux<PersonalCareArtist> findDynamicsInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
