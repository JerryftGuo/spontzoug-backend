...
import com.spontzoug.server.service.ITrainingCoachService;
...
@RestController
@RequestMapping("/api/trainingcoach")
public class TrainingCoachController {
    @Autowired
    private ITrainingCoachService personalCareArtistService;
    @Autowired
    ...
    public void create(@RequestBody TrainingCoach l) {  personalCareArtistService.create(l); }

    @PutMapping("/update")
    @ResponseStatus(HttpStatus.OK)
    public Mono<TrainingCoach> update(@RequestBody TrainingCoach m) {
    ...
    @GetMapping("/id/{id}")
    public ResponseEntity<Mono<TrainingCoach>> findById(@PathVariable("id") String id) {
        Mono<TrainingCoach> m = personalCareArtistService.findById(id);
        HttpStatus s = m!=null ? HttpStatus.OK : HttpStatus.NOT_FOUND;
        return new ResponseEntity<Mono<TrainingCoach>>(m,s);
    }

    @GetMapping("/businessid/{id}")
    public Flux<TrainingCoach> findByBusinessid(@PathVariable("id") String id){
        return personalCareArtistService.findByBusinessid(id);
    }
    @GetMapping("/businessconfig/{id}")
    public Flux<TrainingCoach> findByBusinessConf(@PathVariable("id") String id){
        return personalCareArtistService.findByBusinessid(id);
    }
    @GetMapping("/businessid/{id}/{date}")
    public Flux<TrainingCoach> findByBusinessidInc(
    ...
    public Flux<TrainingCoach> findDynamics(@PathVariable("id") String id){
        return personalCareArtistService.findDynamics(id);
    }
    @GetMapping("/dynamic/{id}/{date}")
    public Flux<TrainingCoach> findDynamicsInc(
            @PathVariable("id") String id,
            @PathVariable("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) Date date){
